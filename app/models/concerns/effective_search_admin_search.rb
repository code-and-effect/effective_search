...
module EffectiveSearchAdminSearch
  ...
  def authorized?(result)
    ...
    if view_context.present?
      return EffectiveResources.authorized?(view_context, :edit, searchable)
    end
  ...
  def effective_path(result)
    ...
    case class_name
    when 'Effective::Page'
      EffectivePages::Engine.routes.url_helpers.edit_admin_page_path(result.searchable)
    when 'Effective::Permalink'
      EffectivePages::Engine.routes.url_helpers.edit_admin_permalink_redirect_path(result.searchable)
    when 'Effective::Post'
      EffectivePosts::Engine.routes.url_helpers.edit_admin_post_path(result.searchable)
    when 'Effective::Event'
      EffectiveEvents::Engine.routes.url_helpers.edit_admin_event_path(result.searchable)
